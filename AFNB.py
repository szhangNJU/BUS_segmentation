...
class _SelfAttentionBlock(nn.Module):
    ...
    def __init__(self, low_in_channels, high_in_channels, key_channels, value_channels, out_channels=None, scale=1, norm_type=None,psp_size=(1,3,6,8)):
        ...
        self.f_key = nn.Sequential(
            ConvBn2d(in_channels=self.in_channels, out_channels=self.key_channels,
                      kernel_size=1, stride=1, padding=0),
            nn.ReLU()
            ,
        )
        self.f_query = nn.Sequential(
            ConvBn2d(in_channels=high_in_channels, out_channels=self.key_channels,
                      kernel_size=1, stride=1, padding=0),
            nn.ReLU(),
        )
    ...
    def __init__(self, low_in_channels, high_in_channels, out_channels, key_channels, value_channels, dropout,
        ...
        self.conv_bn_dropout = nn.Sequential(
           ...
           # ModuleHelper.BatchNorm2d(norm_type=self.norm_type)(out_channels),
            ConvBn2d(out_channels + high_in_channels, out_channels, kernel_size=1, padding=0),
            nn.ReLU(),
