...
class FPNN(nn.Module):
    ...
    def __init__(self,in_ch = 3,out_ch=1,dilation=False,edge_model=False):
        super().__init__()
        self.dilation = dilation
        self.edge_model = edge_model
        self.encoder = se_resnext50_32x4d(1000,in_ch=in_ch)
       ...
       # )
        ...
        self.fusion =  afnb(512, 1024, 256, 256, 256, dropout=0.05, sizes=([1]))
       ...
       # self.decoder5 = Decoder(2048 + 512, 512, 64,OC=True,dilation=(2,4,6))
        self.decoder4 = Decoder(256 + 1024, 512, 256,OC=False,dilation=(4,8,12))
        self.decoder3 = Decoder(256 + 512, 256, 128)
        self.decoder2 = Decoder(128 + 256, 64, 64)
...
#        x[:,2,:,:] = (x[:,2,:,:] - mean[2]) / std[2]
       ...
       # e5 = self.encoder5(e4)  # ; print('e5',e5.size())
        f = self.max(self.fusion(e3,e4))  #; print('center',f.size())
        #f = self.fusion(e4)
        if self.dilation:
        ...
        d4 = self.decoder4(torch.cat([f, e4], 1))  # ; print('d4',d4.size())
        ...
        final = torch.cat((
           ...
           # F.upsample(d5, scale_factor=8, mode='bilinear', align_icorners=False)
        ), 1)
        final = F.dropout2d(final, p=0.20)
        logit = self.logit(final)  # ; print('logit',logit.size())
...
class SENet(nn.Module):
    def __init__(self, block, layers, groups, reduction, in_ch =3, dropout_p=0.2, 
...
def se_resnext50_32x4d(num_classes=1000,in_ch=3):
    model = SENet(SEResNeXtBottleneck, [3, 4, 6, 3], groups=32, reduction=16, in_ch = in_ch,
                  dropout_p=None, inplanes=64, input_3x3=False,
                  downsample_kernel_size=1, downsample_padding=0,
                  num_classes=num_classes)
    # if pretrained is not None:
