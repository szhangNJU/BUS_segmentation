...
class ResNet(nn.Module):
    ...
    def __init__(self, in_ch, block, layers, num_classes=1000,deep_base=False,stem_width=32):
...
def resnet18(in_ch=3,pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(in_ch,BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained and in_ch == 3:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model


def resnet34(in_ch=3,pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(in_ch,BasicBlock, [3, 4, 6, 3], **kwargs)
